...
| status | done, hardware-verified | done, hardware-verified — MiSTer, EverDrive N8 PRO, and since 2026-09-04 **a real MMC5 cartridge** (an ETROM donor board with the ROM written to its flash: boots, ran a 16,029-byte program, kept it across a power cycle after `BACKUP`). `--chr-ram`: emulator, MiSTer, and a Famicom through an EverDrive N8 PRO - but **no MMC5 cartridge ever had character RAM**, so any other machine is untried and it is never the default; the real board above has a flash chip for its picture, so that build cannot apply there | done, hardware-verified | done, both builds hardware-verified (the FM was **listened to**; no machine here can hear it) |
