...
| V3.0 | 4KB | **8KB** | PRG 2バイト ＋ ヘッダ 1バイト |
...
余りは存在しないものとして扱われます。だから**定数を2バイト書き換えるだけ**で領域が広がります。
...
書き換わるのは PRG の2バイトと、iNES ヘッダの 10 バイト目（NVRAM サイズの宣言）1バイトだけです。

| 目標 | 変更量 | 実機での表示 |
|---|---|---|
| V1.0 を 4KB に | PRG 2B ＋ ヘッダ 1B | `4031 BYTES FREE` |
| V2.1A を 4KB に | PRG 2B ＋ ヘッダ 1B | `4030 BYTES FREE` |
| V3.0 を 8KB に | PRG 2B ＋ ヘッダ 1B | `8182 BYTES FREE` |
